...
def test_unit_tensor_creation():
    ...
    vector = Tensor([1, 2, 3])
    assert jnp.array_equal(vector.data, jnp.array([1, 2, 3], dtype=jnp.float32))
    assert vector.shape == (3,)
    assert vector.size == 3

    # Test matrix creation
    matrix = Tensor([[1, 2], [3, 4]])
    assert jnp.array_equal(matrix.data, jnp.array([[1, 2], [3, 4]], dtype=jnp.float32))
...
def test_unit_arithmetic_operations():
    ...
    b = Tensor([4, 5, 6])
    result = a + b
    assert jnp.array_equal(result.data, jnp.array([5, 7, 9], dtype=jnp.float32))

    # Test tensor + scalar (very common in ML)
    result = a + 10
    assert jnp.array_equal(result.data, jnp.array([11, 12, 13], dtype=jnp.float32))

    # Test broadcasting with different shapes (matrix + vector)
    matrix = Tensor([[1, 2], [3, 4]])
    vector = Tensor([10, 20])
    result = matrix + vector
    expected = jnp.array([[11, 22], [13, 24]], dtype=jnp.float32)
    assert jnp.array_equal(result.data, expected)

    # Test subtraction (data centering)
    result = b - a
    assert jnp.array_equal(result.data, jnp.array([3, 3, 3], dtype=jnp.float32))

    # Test multiplication (scaling)
    result = a * 2
    assert jnp.array_equal(result.data, jnp.array([2, 4, 6], dtype=jnp.float32))

    # Test division (normalization)
    result = b / 2
    assert jnp.array_equal(result.data, jnp.array([2.0, 2.5, 3.0], dtype=jnp.float32))

    # Test chaining operations (common in ML pipelines)
    normalized = (a - 2) / 2  # Center and scale
    expected = jnp.array([-0.5, 0.0, 0.5], dtype=jnp.float32)
    assert jnp.allclose(normalized.data, expected)
...
def test_unit_matrix_multiplication():
    ...
    expected = jnp.array([[58, 64], [139, 154]], dtype=jnp.float32)
    assert jnp.array_equal(result.data, expected)
    ...
    expected = jnp.array([14, 32], dtype=jnp.float32)
    assert jnp.array_equal(result.data, expected)
...
def test_unit_shape_manipulation():
    ...
    assert jnp.array_equal(reshaped.data, expected)

    # Test reshape with tuple (alternative calling style)
    reshaped2 = tensor.reshape((3, 2))   # Shape: (3, 2)
    assert reshaped2.shape == (3, 2)
    expected2 = jnp.array([[1, 2], [3, 4], [5, 6]], dtype=jnp.float32)
    assert jnp.array_equal(reshaped2.data, expected2)
    ...
    expected = jnp.array([[1, 4], [2, 5], [3, 6]], dtype=jnp.float32)
    assert jnp.array_equal(transposed.data, expected)

    # Test 1D transpose (should be identity)
    vector = Tensor([1, 2, 3])
    vector_t = vector.transpose()
    assert jnp.array_equal(vector.data, vector_t.data)
    ...
    batch_images = Tensor(jax.random.uniform(key, (2, 3, 4)))  # (batch=2, height=3, width=4)
...
def test_unit_reduction_operations():
    ...
    expected_col = jnp.array([5, 7, 9], dtype=jnp.float32)  # [1+4, 2+5, 3+6]
    assert jnp.array_equal(col_sum.data, expected_col)
    assert col_sum.shape == (3,)

    # Test sum along axis 1 (rows) - feature dimension reduction
    row_sum = matrix.sum(axis=1)
    expected_row = jnp.array([6, 15], dtype=jnp.float32)  # [1+2+3, 4+5+6]
    assert jnp.array_equal(row_sum.data, expected_row)
    assert row_sum.shape == (2,)

    # Test mean (average loss computation)
    avg = matrix.mean()
    assert jnp.isclose(avg.data, 3.5)  # 21/6
    assert avg.shape == ()

    # Test mean along axis (batch normalization pattern)
    col_mean = matrix.mean(axis=0)
    expected_mean = jnp.array([2.5, 3.5, 4.5], dtype=jnp.float32)  # [5/2, 7/2, 9/2]
    assert jnp.allclose(col_mean.data, expected_mean)
    ...
    expected_max = jnp.array([3, 6], dtype=jnp.float32)  # [max(1,2,3), max(4,5,6)]
    assert jnp.array_equal(row_max.data, expected_max)
    ...
    expected_keepdims = jnp.array([[6], [15]], dtype=jnp.float32)
    assert jnp.array_equal(sum_keepdims.data, expected_keepdims)
...
def test_module():
    ...
    assert not jnp.isnan(output.data).any(), "Output contains NaN values"
    assert jnp.isfinite(output.data).all(), "Output contains infinite values"
    ...
    expected = jnp.array([6, 7, 8], dtype=jnp.float32)
    assert jnp.array_equal(result.data, expected)
